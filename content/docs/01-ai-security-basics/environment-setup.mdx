...
前面三章我们建立了 AI 安全的概念框架、模型原理和红队思维。接下来要解决一个更现实的问题：**如果没有稳定、可复现的实验环境，后续所有测试结论都可能不可靠。**

因此，本章不只是“教你把工具装上”，更是帮你建立一套可长期复用的环境基线。你可以把本章理解为一条实践路径：先明确为什么选择云平台，再完成环境配置与验证，最后形成可复用的实验习惯与项目结构。

为保证与配套实验一致，本章默认采用与实验 1.1 相同的基线：**腾讯 Cloud Studio + T4 GPU + `Qwen/Qwen2-1.5B-Instruct`**。
...
这并不意味着本地部署不重要，而是对于课程阶段，云平台更容易把注意力放在“安全能力提升”而非“底层环境排障”上。
